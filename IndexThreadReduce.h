...
public:
                        ...
                        int                                             stepSize = 0    )
	{
		...
		if(stepSize == 0)
            // Calculate step size for given threads number
...
private:
    ...
    // Mutex safe indexes values
    boost::mutex                exMutex;
    // Signal for starting for thread
    boost::condition_variable   todo_signal;
    // Signal for thread done
